...
export function SectionGridPage() {
  ...
  return <div id="top" className="relative overflow-hidden bg-[#f7f3ec] text-[#1e2a38]">
    ...
    <main className="relative z-10">
      ...
      <section id="about" className="bg-white px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-12"><p className="section-label lg:col-span-3">{t["about.label"]}</p><div className="lg:col-span-9"><h2 className="max-w-4xl whitespace-pre-line text-4xl font-medium leading-tight tracking-[-.05em] sm:text-6xl">{t["about.title"]}</h2><div className="mt-10 grid max-w-3xl gap-8 border-t border-[#1e2a38]/15 pt-6 sm:grid-cols-[1fr_2fr]"><span className="text-sm font-semibold text-[#db6049]">MY APPROACH</span><p className="text-lg leading-8 text-[#50606f]">{t["about.body"]}</p></div></div></div></section>
      ...
      <section id="experience" className="bg-[#1d6971] px-5 py-24 text-[#f7f3ec] sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto max-w-6xl"><div className="grid gap-5 lg:grid-cols-12"><p className="section-label text-[#f7f3ec]/60 lg:col-span-3">{t["experience.label"]}</p><h2 className="text-4xl font-medium tracking-[-.05em] sm:text-6xl lg:col-span-9">{t["experience.title"]}</h2></div><ol className="mt-16 border-t border-white/25">{experiences.map((item, index) => <li key={item.role} className="grid gap-4 border-b border-white/25 py-8 md:grid-cols-12"><p className="text-sm font-semibold md:col-span-2">{item.period}</p><div className="md:col-span-6"><h3 className="text-xl font-semibold tracking-[-.03em]">{item.role}</h3><p className="mt-2 text-sm text-white/65">{item.org}</p></div><p className="text-sm leading-6 text-white/75 md:col-span-4">{item.desc}</p><span className="hidden text-right text-sm text-white/50 md:block">0{index + 1}</span></li>)}</ol></div></section>
